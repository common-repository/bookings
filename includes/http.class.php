...
class bookingsHttpRequest {
	...
	private function processHeaders($headers) {
		...
		if (is_string($headers)) {
			...
			$headers=str_replace("\r\n", "\n", $headers);
			// unfold folded header fields. LWS = [CRLF] 1*( SP | HT ) <US-ASCII SP, space (32)>, <US-ASCII HT,
			// horizontal-tab (9)> (RFC 2616 2.2)
	...
	function connect($url, $withHeaders=true, $withCookies=true) {
		...
		// curl_setopt($ch, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0); //avoids error 18 - transfer closed with
