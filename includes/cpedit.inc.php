...
<?php if ($controlpanelOptions) foreach ($controlpanelOptions as $value) {
	...
	<tr align="left">
		...
		<td><input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>"
			...
			<?php if (isset($value['readonly']) && $value['readonly']) echo ' readonly="readonly"'?>
			size="40"
		/></td>
	...
	<tr align="left">
		<th scope="row"><?php echo $value['name']; ?>:</th>
		<td><textarea name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" cols="50" rows="8"/><?php if ( get_option( $value['id'] ) != "") { echo stripslashes (get_option( $value['id'] )); }
