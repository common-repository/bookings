 ...
 * Description: The digital appointment booking solution for you.
 * Author: Zingiri
 * Version: 6.0.4
...
$bookingsRegions['us1']=array('North America, South America & Asia Pacific','bookingsus.zingiri.net/');
$bookingsRegions['eu1']=array('Europe & Africa','bookingseu.zingiri.net/');
...
function bookings_admin_notices() {
	...
	if (!isset($_REQUEST['page']) || !in_array($_REQUEST['page'], array('bookings'))) return;
	...
	if (bookings_active() && !get_option('bookings_lic')) $warnings[]='Start your free Bookings Pro trial now. Simply order a license key valid for 30 days <a href="https://www.zingiri.com/go/cart.php?a=add&pid=159" target="_blank">here</a>.';
	...
	if (!bookings_active()) $warnings[]="Please verify your settings on the Bookings control panel and click 'Connect'";
...
function bookings_deactivate() {
	...
	delete_option('bookings-support-us'); // legacy
}
...
function bookings_admin_header() {
	...
	echo '<script type="text/javascript" src="' . BOOKINGS_URL . 'js/admin.js"></script>';
}
...
function bookings_http($page="index", $params=array()) {
	...
	$wp['bustype']=get_option('bookings_bustype');
	if (is_admin() && get_option('bookings_showcase')) {
...
function bookings_cdn($sub='') {
	if (defined('BOOKINGS_CDN')) return BOOKINGS_CDN;
		...
		$s=(zingiri::isSecure() ? 'https://d173498e4e66d414ff74-516be1fc79a87be931cfbe73f8cfa194.ssl.cf1.rackcdn.com' : 'http://cdn.zingiri.net') . '/bookings/v6/' . ($sub ? $sub . '/' : '');
		return $s;
	}
}

function bookings_active() {
	return get_option('bookings_version') && get_option('bookings_region');
}

function bookings_login() {
	if (get_option('bookings_login')) return get_option('bookings_login');
	else {
...
function bookings_load_js_client() {
	if (BOOKINGS_JSPREFIX == 'src') {
		$url=bookings_url(false);
		echo '<script type="text/javascript" src="' . $url . 'app/bookings/js/' . BOOKINGS_JSPREFIX . '/templatex.jquery.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'lib/fwkfor/js/' . BOOKINGS_JSPREFIX . '/core.jquery.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'app/bookings/js/' . BOOKINGS_JSPREFIX . '/bookx.jquery.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'app/bookings/js/' . BOOKINGS_JSPREFIX . '/hotelx.jquery.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'app/bookings/js/' . BOOKINGS_JSPREFIX . '/helper.jquery.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'lib/fwktec/js/' . BOOKINGS_JSPREFIX . '/jquery.cookie.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'app/bookings/js/' . BOOKINGS_JSPREFIX . '/functions.js"></script>';
		echo '<script type="text/javascript" src="' . $url . 'lib/devbld/js/' . BOOKINGS_JSPREFIX . '/jquery.getUrlParam.js"></script>';
	} else {
		$url=bookings_cdn('js');
		echo '<script type="text/javascript" src="' . $url . 'js/' . BOOKINGS_JSPREFIX . '/client.js"></script>';
	}
}